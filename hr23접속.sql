...
select empno from emp where deptno=30 group by empno;
...
select ename,deptno,sal,sal*0.15+(sal*12) yearsal from emp where deptno=30;
-- 36번 부서번호가 20인 부서의 시간당 임금을 계산하여 출력하라.
-- 단 1달의 근무일수가 12일이고, 1일 근무시간은 5시간이다.
-- 출력양식은 이름, 급여, 시간당 임금(소수이하 첫 번째 자리에서 반올림)을 출력하라.
select ename,sal,round((sal/12)/5,0) timesal from emp where deptno=20;
-- 37번 급여가 $1,500 부터 $ 3,000 사이의 사람은 급여가 15%를 회비로 지불하기로 하였다.
-- 이름, 급여, 회비(소수점 두 자리에서 반올림) 출력하라.
select ename,sal,round(sal*0.15,1) fee from emp where sal>=1500 and sal<=3000;
-- 38번 급여가 $2,000 이상인 모든 사람은 급여가 15%를 경보비로 내기로 하였다.
-- 이름, 급여, 경조비 를 출력하라.
select ename,sal,sal*0.15 fee from emp where sal>=2000;
-- 39번 입사일 부터 지금까지의 날짜수를 출력하라. 부서번호, 이름, 입사일, 현재일, 
-- 근무일수(소수점 이하 절삭), 근무년수, 근무월수(30일 기준), 근무주수를 출력하라.
select deptno,ename,hiredate,sysdate,trunc((sysdate-hiredate),0) workdays,
trunc((sysdate-hiredate)/365,0) workyears,trunc((sysdate-hiredate)/30,0) workmonth,
trunc((sysdate-hiredate)/4,0) workweeks from emp;
-- 40번 모든 사원의 실수령액을 계산하여 출력하라. 
-- 단 급여가 많은 순으로 이름, 급여, 실수령액을 출력하라
-- (실수령액은 급여에 대해 10%의 세금을 뺸 금액)
select ename,sal,sal-(sal*0.1) netpay from emp order by sal desc;
...
select deptno,ename,last_day(hiredate)-hiredate workday from emp;
...
select trunc(avg(sal),0) avg,max(sal) max,min(sal) min,count(sal) count from emp where deptno=10;
...
select deptno,job,count(*) count from emp group by deptno,job order by deptno;
-----------------------------------------------------------------------------230626---------------------------------------------------------------------------------------------
-- 프로시저2 if문
/
declare--=선언하다
